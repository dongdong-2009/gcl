...
//调用static void *Malloc(size_t size);函数时，
//务必将返回值进行判断是否为NULL后再使用
...
int FCGX_Accept_r(FCGX_Request *reqDataPtr)
{
    ...
    while (reqDataPtr) {
        ...
        if(ReadParams(reqDataPtr->paramsPtr, reqDataPtr->in) >= 0)
        {
             ...
             */
            printf("#Finished reading the environment.Leave the connection-retry loop.\n");
            break;
        }
...
//        FCGX_Free(reqDataPtr, 1);
    ...
    {
        printf("#fcgiapp.c,int FCGX_Accept_r(FCGX_Request *reqDataPtr),reqDataPtr is NULL.\n");
    }
    return 0;
}
