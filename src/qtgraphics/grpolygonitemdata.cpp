...
GRPolygonItemData::GRPolygonItemData() :
    fillstyle(NULL)
{
}
GRPolygonItemData::~GRPolygonItemData()
...
void GRPolygonItemData::loadItemData(const QDomElement &element)
{
    ...
    QString vpStr=element.attribute("VP");
    if(!vpStr.isEmpty())
    {
        SetVp(vpStr);
    }
...
void GRPolygonItemData::drawItemData(QPainter *painter)
{
    ...
    painter->drawPolygon(m_Vp.constData(),m_Vp.count());
...
void GRPolygonItemData::changDeviceState(int toState)
{
     return;
}

float GRPolygonItemData::getOpacity() const
{
    return f_Opacity;
}

GRGlobalParameter::GRItemDataType GRPolygonItemData::getItemType() const
{
    return GRGlobalParameter::GR_PolygonType;
}

bool GRPolygonItemData::isCascadePage() const
{
    return this->b_cascadePage;
}

QString GRPolygonItemData::getCascadePageName() const
{
    return this->m_cascadePageName;
}

QRectF GRPolygonItemData::getItemRect() const
{
    return QRectF(this->f_x,this->f_y,this->f_width,this->f_height);
}

void GRPolygonItemData::SetVp(const QString &str)
{
...
void GRPolygonItemData::setPenStyle(const QString &str)
{
    if(str == "3 1")
    {
