...
int XmlMsObjectTemplateManager::loadMessageTempltes()
{
    registerMessageTempltes();
    f_xmlTemplates.clear();

    string sFileTemplatePath = CxFileSystem::mergeFilePath( GclConfig::dataPath() , "file_template" );
    for(typename map<string, pair<string, string> >::const_iterator it = f_xmlTemplateDescripts.begin(); it != f_xmlTemplateDescripts.end(); ++it)
    {
...
std::string XmlMsObjectTemplateManager::toSingleTemplateString(const string &sCommand, const std::map<string, string> & sHeads)
{
    string sFileName = "template_" + sCommand + ".xml";
...
std::string XmlMsObjectTemplateManager::toMultipleTemplateString(const string &sCommand, const std::map<string, string> &sHeads, const std::vector<std::map<string, string> > &sDetailList)
{
    string sFileName = "template_" + sCommand + ".xml";
    const XmlMsObjectTemplate * pTemplate = XmlMsObjectTemplateManager::findTemplate(sFileName);
    if (pTemplate)
...
std::string XmlMsObjectTemplateManager::toMainTemplateString(const string &sCommand, const std::map<string, string> & sHeads, const std::vector<std::map<std::string, std::string> > & sDetailList)
{
    string sFileName = "template_" + sCommand + ".xml";
    const XmlMsObjectTemplate * pTemplate = XmlMsObjectTemplateManager::findTemplate(sFileName);
    if (pTemplate)
...
string XmlMsObjectTemplateManager::toMainTemplateString(const string &sCommand, const std::map<string, string> &sHeads, const std::map<string, std::map<string, string> > & sDetails)
{
    string sFileName = "template_" + sCommand + ".xml";
    const XmlMsObjectTemplate * pTemplate = XmlMsObjectTemplateManager::findTemplate(sFileName);
    if (pTemplate)
...
bool XmlMsObjectTemplateManager::createSimpleTemplate(const string &sCommand, const std::map<string, string> & sHeads, string &sFileName)
{
    ...
    if (pTemplate)
    {
        ...
        string sPath = CxFileSystem::mergeFilePath( GclConfig::tempPath() , "send" );
        CxFileSystem::createDirMultiLevel(sPath);
        sFileName =  CxFileSystem::mergeFilePath(sPath, CxTime::toString(CxTime::currentMsepoch(), 0, 0, 0)+"_"+ sCommand + ".xml");
...
bool XmlMsObjectTemplateManager::createDetailTemplate(const string &sCommand, const std::map<string, string> & sHeads, const std::vector<std::map<std::string, std::string> > & sDetailList,std::string &sFileName)
{
    string sName = "template_" + sCommand + ".xml";
    const XmlMsObjectTemplate * pTemplate = findTemplate(sName);
    if (pTemplate)
