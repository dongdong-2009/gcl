...
void GeneralBusiness::toContext(CxISetSkv &context) const
{
    BusinessBase::toContext(context);
}

void GeneralBusiness::fromContext(const CxIGetSkv &context)
{
    BusinessBase::fromContext(context);
...
void GeneralBusiness::describeSelf(CxDescribeUnit *oDescribeUnit) const
{
    BusinessBase::describeSelf(oDescribeUnit);
...
void GeneralBusiness::toReport(std::vector<string> &sReports) const
{
    BusinessBase::toReport(sReports);
...
void GeneralBusiness::businessAfterStart()
{
    BusinessBase::businessAfterStart();

    CxMsObjectDataManager::addObserver(this);
}

void GeneralBusiness::businessAfterStop()
{
    BusinessBase::businessAfterStop();

    CxMsObjectDataManager::removeObserver(this);
}

void GeneralBusiness::registerProtocol(ProtocolBase *oProtocol)
{
    BusinessBase::registerProtocol(oProtocol);
...
void GeneralBusiness::unregisterProtocol()
{
    BusinessBase::unregisterProtocol();
...
void GeneralBusiness::received_message_command(const PsmProtocol *oPsm, const char *sCommand, const char *sParam, int iParamLength, const PsmAttach *oAttach)
{
    assert(oPsm != NULL);
...
void GeneralBusiness::received_file_write(const PsmProtocol *oPsm, const CxFileSystem::PathInfo &pathInfo, const std::vector<std::string> &fileData, const PsmAttach *oAttach)
{
//    assert(oPsm != NULL);
...
void GeneralBusiness::received_realtime_data_request(const PsmProtocol *oPsm, const PsmAttach *oAttach)
{
    assert(oPsm != NULL);
...
void GeneralBusiness::received_realtime_data_post(const PsmProtocol *oPsm, int iParamType, const char *pData, int iDataLength, int iParamCount, const PsmAttach *oAttach)
{
    assert(oPsm != NULL);
...
int GeneralBusiness::sendMessage(const string & sCommand, const string &sParam)
{
    int r = 0;
...
int GeneralBusiness::sendFile(const string &sFilePath)
{
    int r = 0;
