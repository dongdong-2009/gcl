...
size_t f_monsbPublishedIndex = GM_UINT_MAX;
size_t f_ycaddPublishedIndex = GM_UINT_MAX;
size_t f_strawPublishedIndex = GM_UINT_MAX;
...
public:
    ...
    int update(const std::map<string, string> &row) {
        ...
        {
            vector<string> v = CxString::split(sSubscibes,',');
        ...
        else
        {
            ...
            if (sType == g_fn_getMeasureTypeName(_measureType) && iInterval>=1000)
            {
...
//            cxDebug() << "fn_postPublishPacket " << oPublishPacket->structTypeId << " " << oPublishPacket->dataCount << " " << oPublishPacket->terminalId << " , ";
...
void BusMeasurePublisher::stop()
{
    if (f_oScanMeasurePublishThread)
    {
        f_oScanMeasurePublishThread->stop();
