...
static map<int , FILE*> f_measureChangedFiles;

size_t RtdbLogFile::loadMeasureChangedIndex(const std::string &sFileName)
{
...
size_t RtdbLogFile::loadMonsbChangedIndex()
{
    return loadMeasureChangedIndex(f_sMonsbChangedIndexFileName);
}

size_t RtdbLogFile::loadYcaddChangedIndex()
{
    return loadMeasureChangedIndex(f_sYcaddChangedIndexFileName);
}

size_t RtdbLogFile::loadStrawChangedIndex()
{
    return loadMeasureChangedIndex(f_sStrawChangedIndexFileName);
}

int RtdbLogFile::saveMeasureChangedIndex(size_t iOldIndex, size_t iNewIndex, FILE *oFile)
{
...
int RtdbLogFile::saveMonsbChangedIndex(size_t iOldIndex, size_t iNewIndex)
{
    return saveMeasureChangedIndex(iOldIndex, iNewIndex, f_oMonsbChangedIndexFile);
}

int RtdbLogFile::saveYcaddChangedIndex(size_t iOldIndex, size_t iNewIndex)
{
    return saveMeasureChangedIndex(iOldIndex, iNewIndex, f_oYcaddChangedIndexFile);
}

int RtdbLogFile::saveStrawChangedIndex(size_t iOldIndex, size_t iNewIndex)
{
    return saveMeasureChangedIndex(iOldIndex, iNewIndex, f_oStrawChangedIndexFile);
}

int RtdbLogFile::start()
{
...
int RtdbLogFile::stop()
{
    for(typename std::map<int, FILE*>::const_iterator it = f_measureChangedFiles.begin(); it != f_measureChangedFiles.end(); ++it)
    {
...
std::string RtdbLogFile::getMeasureValue(const int &value)
{
    return CxString::toString(value);
}
std::string RtdbLogFile::getMeasureValue(const double &value)
{
    return CxString::toString(value);
}
std::string RtdbLogFile::getMeasureValue(const StrawValue &value)
{
...
FILE *RtdbLogFile::getMeaureChangedFile(int iMeasureId)
{
    FILE *oFile = NULL;
    map<int, FILE *>::iterator it = f_measureChangedFiles.find(iMeasureId);
    ...
    if (oFile == NULL)
    {
        ...
        if (oFile != NULL)
        {
            f_measureChangedFiles[iMeasureId] = oFile;
            return oFile;
        }
    }
    return oFile;
...
int RtdbLogFile::checkMeaureChangedFiles()
{
    static int iTime = 0;
...
std::string
RtdbLogFile::getMeasureChanngedText(int iMeasureId, const int &value, int iQuality, msepoch_t dtChangedTime, int iChangedSourceId, int iChangedReasonId)
{
    return CxString::format(f_sMonsbChangedIndexText.c_str(), iMeasureId, value, iQuality, dtChangedTime, iChangedSourceId, iChangedReasonId);
}
std::string
RtdbLogFile::getMeasureChanngedText(int iMeasureId, const double &value, int iQuality, msepoch_t dtChangedTime, int iChangedSourceId, int iChangedReasonId)
{
    return CxString::format(f_sYcaddChangedIndexText.c_str(), iMeasureId, value, iQuality, dtChangedTime, iChangedSourceId, iChangedReasonId);
}
std::string
RtdbLogFile::getMeasureChanngedText(int iMeasureId, const StrawValue &value, int iQuality, msepoch_t dtChangedTime, int iChangedSourceId, int iChangedReasonId)
{
    int datalen = strlen(value.value) > ci_straw_length ? ci_straw_length : strlen(value.value);
    std::string sValue = std::string(value.value, datalen);
    return CxString::format(f_sStrawChangedIndexText.c_str(), iMeasureId, sValue.c_str(), iQuality, dtChangedTime, iChangedSourceId, iChangedReasonId);
}
